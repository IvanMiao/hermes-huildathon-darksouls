...
- ElevenLabs 与 Cloudflare 的真实集成；Cloudflare 同时使用 Pages、Tunnel、D1 和 R2
- 程序化战斗角色和攻击预警
...
- 动态 sprite sheet、运行时音乐合成、复杂背景生成
- WebSocket；active run 使用单向 SSE，完成后由 D1 承担 durable replay
...
Manager 合并 DraftGameRecipeV0（内含 BossSpec）
  ↓
Audio Producer → ElevenLabs voice.mp3 + original boss-music.mp3
  ↓
...
| Audio | approved voice + music direction | `VoiceArtifact` + `MusicArtifact` |
...
Cloudflare Pages
  ...
  └── /games/:runId  固定 Three.js runtime
           │
           │ completed-run evidence queries
           ▼
Pages Functions
  ├── D1: studio_runs / generated_audio
  ├── R2: voice / music artifacts
  └── ElevenLabs voice / music generation
           ▲
           │ protected writes + file uploads
           │
Cloudflare Tunnel
           │
Local Studio Runner
...
Pages 提供稳定的公开壳；Tunnel 只暴露现场运行的 Hermes runner。已经生成的游戏只依赖 Pages Functions + D1 + R2，因此即使本地 runner 暂时掉线，评委仍可打开历史游戏。

### 一次 run 的请求路径

1. Pages 上的 Studio 调用 Tunnel 暴露的 `/api/studio/runs`，立即获得现场 job ID。
2. 本地 runner 创建 append-only run，并让 Hermes 开始 production workflow。
3. 每个 agent/tool step 先写入本地 append-only run store，并通过受保护的同源 SSE snapshot 推送给 active Control Room。
4. Control Room 对密集事件按真实顺序呈现；断线或滚动部署时退回短轮询，不修改事件时间戳。
5. QA 通过后，runner 调用受 token 保护的 Pages evidence API；Pages Function 用 ElevenLabs 生成音频并写入 R2。
6. runner 把最终 recipe、events、artifacts 和 QA report 写入 D1，随后 job 才标记为完成。
7. `/control-room/:runId` 从 D1 重放完成后的 durable evidence；`/games/:runId` 读取 D1 recipe 和同源 R2 artifact URL 启动固定 runtime。

选择 hybrid 路径是因为 active job 必须立即可见，而 D1 只保存已经通过发布门的 durable evidence。当前实现不把完成后的一次性 D1 mirror 误称为实时总线：现场状态来自 runner SSE，历史状态来自 D1。
...
```text
D1 studio_runs
  run_id, status, input_text, recipe_json,
  events_json, artifacts_json, qa_report_json,
  voice_json, music_json, mirrored_at

D1 generated_audio
  run_id, voice_json, music_json, updated_at
...
- D1 存产品状态和可验证证据，不存 agent 的隐式 chain-of-thought。
- 每个 event 记录输入/输出摘要、耗时、成本、artifact 指针。
- 大文件写入 R2；D1 只保存 JSON metadata 和稳定的同源 URL。
- `run_id` 是幂等写入键；重复 mirror 覆盖同一条记录，不创建重复 run。
- 浏览器只开放 GET；runner 写入和历史迁移共享独立的 `STUDIO_INTEGRATION_TOKEN`。

Cloudflare Pages Functions 可以直接绑定 D1 与 R2；本项目通过同一个 `/api/evidence/*` 路由访问两者，避免向浏览器暴露存储凭据。
...
- Pages Function 使用 ElevenLabs 官方 HTTP API。
...
- API key 只存在 Pages 服务端环境变量中，绝不进入 runner 或 Vite bundle。
...
这样做比把 Hermes 塞进 Worker 更稳：runner 继续运行在正常 Node/Python 环境，但接收不可信产品输入的 Hermes specialist 必须运行在 Bubblewrap 中，隐藏用户 home 和项目目录，并禁用文件、终端、代码执行、plugins 与 MCP；Cloudflare 只负责公开入口和静态交付。需要本地文件或浏览器能力的确定性 QA 仍由 Manager 控制的非模型代码执行，不能授权给 specialist。
...
3. Pages 绑定 D1 `SOULLOOM_DB`、R2 `SOULLOOM_ARTIFACTS`，并配置 runner 与 evidence secrets。
...
- Cloudflare D1：完成 run、recipe 与 QA evidence
- Cloudflare R2：voice 与 music artifacts
- Cloudflare Pages Functions：evidence API、ElevenLabs 服务端调用和 runner 反向代理
...
- 把 Hermes Kanban、D1 状态和自建 orchestrator 同时作为流程真相源

唯一流程真相源应是 Hermes 当前 run；D1 是其完成后的外部可视化和证据镜像。
...
40–60s 三次成功交付、公开 URL、ElevenLabs 事件和 D1/R2 记录
```
...
音乐按 run 批量生成并进入发布 evidence；浏览器运行时不做实时音乐合成。

Buildathon 版本：

- 保留一套 Soulloom 标志性 Boss 音乐，仅用于 playground 与旧 recipe 迁移。
- runtime 根据游戏状态切换音乐片段。
- Audio Producer 现场生成并绑定 Boss 语音。
- Audio Producer 为每条新 run 生成 Music v2 score；voice 与 music evidence
  都成功 mirror 后才解锁游戏 URL。生成失败显示 evidence sync failure，不把
  House Track 冒充成本次生成。
...
Music v2 的 provider prompt 只接收受控的 archetype、camera mood 与 arena
词表映射。Hermes 生成的 Boss 名称、称号和 motif 原文保留在 Evidence direction
中供审计，但不直接注入 provider prompt，避免自由文本触发版权分类或不稳定的
composition-plan 行为。

### 12.4 音乐 QA

音乐 artifact 只有满足以下条件才可进入新 run 的发布 recipe：
...
- 文件已写入 Cloudflare R2，recipe 使用可访问的同源 HTTPS URL。

最终产品原则：

> UI 保持极简；Agency 通过产物、拒绝和返工被看见；souls-like 不作为模仿对象，而成为 Soulloom 自己的叙事语法；每次发布都携带可审计的原创 voice 与 music evidence。
