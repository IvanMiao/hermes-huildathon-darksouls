...
- WebSocket；active run 使用单向 SSE，完成后由 Convex 承担 durable replay
...
4. 每个 agent/tool step 先写入本地 append-only run store，并通过受保护的同源 SSE snapshot 推送给 active Control Room。
5. Control Room 对密集事件按真实顺序呈现；断线或滚动部署时退回短轮询，不修改事件时间戳。
6. 图片和音频上传到 Convex File Storage，并将 storage ID 写入 `artifacts`。
7. QA 通过后把完整 run mirror 到 Convex，并更新为 `published`。
8. `/control-room/:runId` 可从 Convex 重放完成后的 durable evidence；`/games/:runId` 读取 recipe 和 artifact URL 启动固定 runtime。

选择 hybrid 路径是因为 active job 在 Convex run 文档建立前就必须立即可见，而最终发布仍要求 Convex evidence mirror 成功。后续若把 events/artifacts 拆成独立 Convex 表，可以把 active SSE 替换为 subscription；当前实现不把完成后的一次性 mirror 误称为实时 subscription。
...
- 文件已写入 Convex File Storage，recipe 使用可访问的 HTTPS URL。
