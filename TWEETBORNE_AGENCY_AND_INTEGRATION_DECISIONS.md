...
- AI as Agency 主赛道
- 固定 Phaser runtime + 受约束的 `BossSpec`
...
1. Cloudflare Pages 部署 Vite Control Room 和 Phaser runtime。
...
## 11. Tweetborne House Style：建立自己的黑暗神话语法
...
正确做法是抽取其有效的叙事特征，再形成 Tweetborne 自己的世界规则、语言规则、视觉语法和声音语法。
...
```json
{
  "styleProfile": "tweetborne_gothic_v1",
...
- 预生成并缓存一套 Tweetborne 标志性 Boss 音乐。
...
> UI 保持极简；Agency 通过产物、拒绝和返工被看见；souls-like 不作为模仿对象，而成为 Tweetborne 自己的叙事语法；音乐成为版本化的 House Sound，而不是每次 run 都必须成功的随机步骤。
