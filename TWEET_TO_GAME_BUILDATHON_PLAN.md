# Tweetborne：Hermes Tweet-to-Game Buildathon 方案
...
Tweetborne 不是一个普通的“AI 生成小游戏”工具，而是一间由 Hermes 驱动的微型自主游戏工作室。
...
- Phaser 3
- 单个 `BattleScene`
- Phaser Graphics、粒子、相机震动和简单几何碰撞
- 桌面键盘优先
...
当前项目没有既有 3D 模型和成熟模板。Three.js/R3F 会引入模型、骨骼动画、相机、阴影、碰撞和 GPU 兼容风险；Godot Web 则有导出体积、浏览器音频和现场迭代风险。
...
- 扇形、直线和圆环预警容易绘制
- 没有模型导入和骨骼动画风险
- 可以通过剪影、粒子、冻结帧、震屏和音频制造足够强的舞台效果

只有在已有成熟 3D 模板和可用模型时，才考虑 R3F。
...
| 时间 | 交付 |
|---|---|
| 00:00–00:45 | 定范围、Vite/Phaser 脚手架、BossSpec Schema |
...
> 如果互联网发生的每件事，都能立刻变成一个可玩的世界呢？Tweetborne 不是让 AI 写一个游戏。我们给 Hermes 一条 tweet，它会自己组建工作室：编剧理解事件，战斗设计师发明机制，配音导演创造角色，QA 拒绝不可玩的版本。最后，它交付一个任何人都能玩的线上 Boss 战。
...
> 普通生成器在输出代码时停止。Tweetborne 会拒绝不可玩的产品，找到责任人，要求返工，重新验收，并交付一个真实 URL。We don’t generate game code. We operate an autonomous game studio.
